...
/**
 * Generates an array of uniform random numbers based on the previously set seed.
 * @param k
 * @return
 */  
  public final synchronized double[] randomArrayDouble(int k) {
...
/**  
 * Returns an array random number from a Gaussian distribution based on the previously set seed.  
 * @param k
 * @return
 */
  public final synchronized double[] gaussianArray(int k) {
    double[] values = new double[k];
    for(int i=0; i<k; i++) {
      values[i] = nextGaussian();
    }
    return values;
  }
