...
import com.AandR.defaultHdf5.HDF5DataObject;
import com.AandR.defaultHdf5.ISavableData;
...
public class HDF5Worker {
    ...
    private static int dataType = HDF5DataConstants.ARRAY_DOUBLE;
    ...
    public static void close() {
        try {
            ...
            //new ErrorDialog("HDF5 Error", "An error occurred while closing \"" + h5File.getPath() + "\"", e);
        }
    }
    ...
    protected static void createGroups(String filename, ArrayList<LinkedHashMap<String, String>> loops, ArrayList<String> pluginNames) throws Exception {
        initialize();
        h5File = new H5File(filename);
        h5File.createFile(filename, FileFormat.FILE_CREATE_DELETE);
        h5File.open();
        ...
        // Add Loop Groups
        int runCount = loops == null || loops.size() < 1 ? 1 : loops.size();
        String[] groupNames;
        Group runGroup, pluginGroup;
        DecimalFormat df;
        if (runCount < 10) {
        ...
        for (int i = 0; i < runCount; i++) {
            runGroup = h5File.createGroup("/Run " + df.format(i + 1), null);
            for (String p : pluginNames) {
                //groupNames = p.getSavableOutputKeys();
                //if(groupNames==null) continue;

                // Create plugin group
                pluginGroup = h5File.createGroup(p, runGroup);

                // initialize counter map.
                //for(String dsName : groupNames) {
                //counterMap.put(pluginGroup.getFullName() + "/" + dsName, initialIndex);
                counterMap.put(pluginGroup.getFullName(), initialIndex);
            //}
            }
        }
        h5File.close();
    ...
    public static Group createGroup(String groupPath) throws Exception {
        return createGroup(groupPath, null);
    }

    private static Group createGroup(String groupName, Group group) throws Exception {
        if(groupName.length()<1) return group;
        
        String[] groupSplit = groupName.split("/");
        String subGroupName = group == null ? "" : group.getFullName();
    ...
    public static void appendData(String groupPath, String datasetName, Object data, double time) {
        try {
            if (data instanceof ISavableData) {
                appendData(groupPath, datasetName, (ISavableData) data, time);
            ...
            //new ErrorDialog("HDF5 Error", "An error occurred while appending LatFile data.", e);
        }
    }
     ...
     * @throws Exception
     */
    public static void appendData(String group, String dsName, ISavableData si, double time) throws Exception {
        HDF5DataObject latDataObject;
        String mapKey;
        Group g2 = (Group) h5File.get(group);

        String thisGroup;
        for (String key : si.createVariablePaths()) {
            int k = key.lastIndexOf("/");
            if(k>0) {
                thisGroup = group + "/" + key.substring(0, k);
                dsName = key.substring(k+1);
            } else {
                thisGroup = group;
                dsName = key;
            }
            if ((Group) h5File.get(thisGroup) == null) {
                createGroup(thisGroup);
            }
//
            latDataObject = si.getDataFor(key);
            mapKey = thisGroup + "/" + dsName;
            ...
            if (latDataObject.getWriteMode() == HDF5DataObject.APPEND) {
                appendData(thisGroup, dsName, latDataObject.getData(), time);
            } else {
                writeData(thisGroup, dsName, latDataObject.getData(), time);
            }
        }
    }

    public static void appendData(String gid, String dsName, File[] data, double time) {
        System.out.println("HDF5Worker DataType - File type under development");
    /*
    ...
    public static void writeData(String groupPath, String datasetName, Object data, double time) {
        try {
            if (data instanceof ISavableData) {
                writeData(groupPath, datasetName, (ISavableData) data, time);
            } else if (data instanceof Integer) {
