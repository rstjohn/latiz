...
/*
 * JEditTextArea.java - jEdit's text component Copyright (C) 1999 Slava Pestov You may use and modify this package for any purpose. Redistribution is permitted, in both source and binary form,
...
import com.AandR.library.io.AsciiFile;
...
/**
 * jEdit's text area component. It is more suited for editing program source code than JEditorPane, because it drops the unnecessary features (images, variable-width lines, and so on) and adds a whole
 * bunch of useful goodies such as:
 * <ul>
 ...
 * 
 * @author Slava Pestov
 * @version $Id: JEditTextArea.java,v 1.3 2007/09/12 18:04:07 stjohnr Exp $
 */
public class JEditTextArea extends JPanel {
    ...
    private boolean codeCompletionUsed, autoCompletionUsed;
    private File currentFile;
    private FindReplaceDialog findReplaceDialog;
    private String currentDirectory,  clipboardString,  originalFile;
    ...
    public JEditTextArea(TextAreaDefaults defaults) {
        ...
        lineNumberPanel.setTextFont(painter.getFont());

        // Initialize the GUI
        setLayout(new ScrollLayout());
        add(CENTER, painter);
        add(LEFT, lineNumberPanel);
        add(RIGHT, verticalScrollBar = new JScrollBar(Adjustable.VERTICAL));
        add(BOTTOM, horizontalScrollBar = new JScrollBar(Adjustable.HORIZONTAL));

        // Add some event listeners
        verticalScrollBar.addAdjustmentListener(new AdjustHandler());
        horizontalScrollBar.addAdjustmentListener(new AdjustHandler());
    ...
    public String getLinesForCurrentDiv(boolean shouldAdvanceCaret) {
        int currentLine = getCaretLine()+1;
        TreeSet<Integer> divs = lineNumberPanel.getDivLineNumbers();
        if(divs==null || divs.isEmpty()) {
            return getText();
        }

        Integer startLine, endLine;
        if(divs.contains(currentLine)) {
            startLine = currentLine;
            endLine = divs.higher(startLine);
            if(endLine==null) {
            ...
            startLine = divs.floor(currentLine);
            if(startLine == null) startLine = 1;
            endLine = divs.ceiling(currentLine);
            if (endLine == null) {
                endLine = getLineCount();
            } else {
                endLine = endLine - 1;
        ...
        // Offsets require Java line indexing (zero-based)
        int startOffset = getLineStartOffset(startLine-1);
        int endOffset = getLineEndOffset(endLine-1);
        try {
            String textSegment;
            textSegment = document.getText(startOffset, endOffset-startOffset);
            if (shouldAdvanceCaret) setCaretPosition(Math.min(endOffset, document.getLength()));
            return textSegment;
    ...
    public void setTextFont(Font font) {
        ...
        lineNumberPanel.setTextFont(font);
        recalculateVisibleLines();
    }

    public final void addEditListener(JEditListener listener) {
        listeners.add(listener);
    }

    public final void removeEditListener(JEditListener listener) {
    ...
    public final void fireFileUpdated(boolean hasTextChanged) {
        for (int i = 0; i < listeners.size(); i++) {
            listeners.get(i).fileChanged(hasTextChanged);
        }
    }

    protected LineNumberPanel getLineNumberPanel() {
    ...
    public void updateScrollBars() {
        if (verticalScrollBar != null && visibleLines != 0) {
            lineNumberPanel.setLineNumberRange(firstLine+1, Math.min(getLineCount()+1,firstLine+1+visibleLines));
            verticalScrollBar.setValues(firstLine, visibleLines, 0, getLineCount());
            verticalScrollBar.setUnitIncrement(2);
            verticalScrollBar.setBlockIncrement(visibleLines);
        }

        int width = painter.getWidth();
        if (horizontalScrollBar != null && width != 0) {
            horizontalScrollBar.setValues(-horizontalOffset, width, 0, width * 5);
            horizontalScrollBar.setUnitIncrement(painter.getFontMetrics().charWidth('w'));
            horizontalScrollBar.setBlockIncrement(width / 2);
        }
    }
...
//  int oldFirstLine = this.firstLine;
        ...
        if (firstLine != verticalScrollBar.getValue()) {
            updateScrollBars();
        }
        painter.repaint();
    }
...
//  int oldVisibleLines = visibleLines;
        visibleLines = painter.getHeight() / lineHeight;
        updateScrollBars();
    }
    ...
    public void setHorizontalOffset(int horizontalOffset) {
        ...
        if (horizontalOffset != horizontalScrollBar.getValue()) {
            updateScrollBars();
        }
        painter.repaint();
    }
...
//  int oldFirstLine = this.firstLine;
    ...
    public boolean scrollTo(int line, int offset) {
        ...
        lineNumberPanel.updateLineNumbers(newFirstLine+1, Math.min(lineCount, newFirstLine+visibleLines+1));
...
//    Toolkit toolkit = painter.getToolkit();
    ...
    public void gotoLine(int line) {
        Element elemen3 = document.getDefaultRootElement();
        Element elemen4 = elemen3.getElement(line);
        select(elemen4.getStartOffset(), elemen4.getEndOffset());
    }
    ...
    public void loadAsciiFile(File file) {
        currentFile = file;
        try {
            String[] lines = AsciiFile.readLinesInFile(currentFile);
            ...
            fireFileUpdated(false);
            lineNumberPanel.updateLineNumbers(1, Math.min(lines.length+1,75));
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void saveAsciiFileAs(File file) {
        saveAsciiFile(file, true);
    }

    public void saveAsciiFile() {
        if (currentFile == null) {
        } else {
            saveAsciiFile(currentFile, true);
        }
    }

    private void saveAsciiFile(File file, boolean printInfo) {
        try {
            currentFile = file;
            originalFile = getText();
            AsciiFile.write(currentFile, new String[]{getText()});
            fireFileUpdated(false);
            if (printInfo) {
                System.out.println("File Saved As..." + currentFile.getPath());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void processKeyEvent(KeyEvent evt) {
        ...
        switch (evt.getID()) {
            case KeyEvent.KEY_TYPED:
                String s = new String(new char[]{evt.getKeyChar()});
                if (s.equals(" ") && evt.isControlDown()) {
                    evt.consume();
                    return;
                }
    ...
    protected void fireCaretEvent() {
        Object[] localListeners = eventListenerList.getListenerList();
        for (int i = localListeners.length - 2; i >= 0; i--) {
            if (localListeners[i] == CaretListener.class) {
                ((CaretListener) localListeners[i + 1]).caretUpdate(caretEvent);
            }
        }
    }
...
//            return dim;
    ...
    class MouseWheelHandler implements MouseWheelListener {
        ...
        public void mouseWheelMoved(MouseWheelEvent e) {
            int currentFirstLine = getFirstLine();
            int lastLine = getLineCount() - 1;
            int newFirstLine = currentFirstLine + e.getWheelRotation();
            newFirstLine = newFirstLine < 0 ? 0 : (newFirstLine > lastLine ? lastLine : newFirstLine);
    ...
    class AdjustHandler implements AdjustmentListener {
        ...
        public void adjustmentValueChanged(final AdjustmentEvent evt) {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                ...
                public void run() {
                    if (evt.getAdjustable() == verticalScrollBar) {
                        int firstLine = verticalScrollBar.getValue();
                        setFirstLine(firstLine);
                        lineNumberPanel.updateLineNumbers(firstLine+1, Math.min(getLineCount(), firstLine+visibleLines+1));
                    } else {
                        setHorizontalOffset(-horizontalScrollBar.getValue());
                    }
                }
            });
        }
...
//    
        ...
        private void doDoubleClick(int line, int offset, int dot) throws BadLocationException {
            // Ignore empty lines
    ...
    public boolean isCodeCompletionUsed() {
        return codeCompletionUsed;
    }

    public void setCodeCompletionUsed(boolean codeCompletionUsed) {
        this.codeCompletionUsed = codeCompletionUsed;
    }

    public boolean isAutoCompletionUsed() {
        return autoCompletionUsed;
    }

    public void setAutoCompletionUsed(boolean autoCompletionUsed) {
        this.autoCompletionUsed = autoCompletionUsed;
    }
}
